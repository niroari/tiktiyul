...
- **Bus class slot UI (appendix ו):** Instead of 3 always-visible class dropdowns, the bus row shows 1 dropdown + a "+" button. Clicking "+" adds a 2nd slot; another "+" adds a 3rd. Slots 2–3 have a "×" button to remove them. Tracked via `bus.classSlotCount` (default 1, max 3). Backward compat: on load, `classSlotCount` is derived from the number of non-empty `classSelections` values.
- **Class split — default 2 parts:** `splitFClass` now initializes `fSplits[cls] = ['', '']` (2 parts). A "+ חלק ג׳" button appears below the last part row if `parts.length < 3`. Clicking it calls `addFSplitPart(cls)` which pushes a 3rd entry. Existing 3-part splits loaded from Firestore still render all 3 rows.
- **Bus signs font size (שילוט):** `buildSignHtml` now detects 3 classes in the sign text (≥2 commas) and drops font size 40% (230pt → 138pt). 2 classes (1 comma, length > 8) keeps 173pt. 1 class keeps 230pt.
- **Map embed research (decided to skip):** Checked Israel Hiking Map (israelhiking.osm.org.il) and Amudanan (amudanan.co.il). Neither sets X-Frame-Options, so iframe embedding is technically possible. Israel Hiking has a public share URL format (`/share/{id}`) and Swagger API. Amudanan has no documented URL parameters or API. Decided not to add a map section for now.
